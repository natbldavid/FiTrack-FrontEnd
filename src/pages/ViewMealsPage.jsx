...
function MealItemRow({ item }) {
  const imageSrc = item?.foodImageUrl || PLACEHOLDER_IMAGE

  return (
    <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white px-3 py-3">
      <img
        src={imageSrc}
        alt={item?.foodName || 'Meal item'}
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500">
          <span>{formatNumber(item?.calories)} cal</span>
          <span>{formatMacroNumber(item?.protein)}p</span>
          <span>{formatMacroNumber(item?.carbs)}c</span>
          <span>{formatMacroNumber(item?.fat)}f</span>
        </div>
      </div>
    </div>
  )
...
function ViewMealsPage() {
  ...
  useEffect(() => {
    ...
    const loadMeal = async () => {
      if (!mealId) {
        setErrorMessage('Meal not found.')
        setIsLoading(false)
        return
      }
      ...
      try {
        ...
        if (!isMounted) return
        setMeal(null)
        setErrorMessage('Unable to load meal.')
      } finally {
        if (isMounted) {
          setIsLoading(false)
        }
  ...
  const handleLogMeal = async () => {
    try {
      ...
      if (!mealItems.length) {
        setLogErrorMessage('This meal has no items to log.')
        return
      }
      ...
      setLogErrorMessage('Unable to log meal.')
    } finally {
      setIsLogging(false)
    }
  }
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      const uploadResult = await uploadImageToCloudinary(file, 'fitrack/meals')
      ...
      const updatedMeal = await updateMeal(meal.id, payload)

      setMeal((currentMeal) => ({
        ...currentMeal,
        ...updatedMeal,
        imageUrl: updatedMeal?.imageUrl || nextImageUrl,
      }))
